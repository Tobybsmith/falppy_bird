...
void draw()
{
  ...
  if (mode == 0) //in the menus
  {
    ...
    if (keyPressed && key == 's') //if the player presses x
    {
      sans = true;
  ...
  } else if (mode == 1)
  {
    ...
    if (colourPalette % 3 == 2) //if palette is 3, also draw tertiary background images
    {
      image(sky3, sky3X, 0);
      image(sky4, sky4X, 0);
    }
    ...
    if (!isAlive)
    {
      if (sans) //if the player died on sans mode, close the program for dramatic effect
      {
        exit();
      }
